...
[Government of Canada's Open Data website](http://open.canada.ca/en/open-data) [@horses1]. 
...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig-horse-pops-plot width=70% fig-pos="H"}
...
We present the standard deviations in @tbl-horse-population-std.

```{python}
#| label: tbl-horse-population-std
#| fig-cap: "Standard deviation of historical (1906-1972) horse populations for each Canadian province."
...
Additionally, note that in @tbl-horse-population-std we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pops-plot.

![Horse populations for the province with the largest standard deviation
](../results/horse_pops_plot_largest_sd.png){#fig-horse_pop_largest_sd width=70% fig-pos="H"}

In @fig-horse_pop_largest_sd we zoom in and look at the province of `{python} largest_sd` which had the largest spread of values in terms of standard deviation.
